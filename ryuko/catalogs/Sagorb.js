...
var ryukoValue;
try {
  global.client.ryukoPath = join(global.client.mainPath, "../configs/Sagor.json");
...
try {
  if (!global.config.BOTNAME) {
    logger.error(`please enter your bot name in ${chalk.blueBright('Sagor.json')} file`);
    process.exit(0);
  }
  if (!global.config.PREFIX) {
    logger.error(`please enter your bot prefix in ${chalk.blueBright('Sagor.json')} file`)
  }
  if (global.config.OWNER != "100048281103451") {
    logger.error(`detected : OWNER was changed at ${chalk.blueBright('Sagor.json')}`);
    process.exit(0);
  }
  if (packages.author != "Sagor-islam") {
    logger.error(`detected : author was changed at ${chalk.blueBright('package.json')}`);
    process.exit(0);
  }
  if (packages.name != "Sagor-islam") {
...
try {
  var appStateFile = resolve(join(global.client.mainPath, "../../Sagorstate.json"));
  ...
  console.log(chalk[`${cnslEvent.logger.colors.cmdStarter}`](`${cnslEvent.logger.strings.cmdStarter}`));
        for (const command of listCommand) {
          try {
            ...
            const configures = require(`../../Sagor.json`);
            ...
            if (envConfig) {
              ...
              var ryukoPath = require('../configs/Sagor.json');
...
(async () => {
  try {
    ...
    logger(`deployed ${chalk.blueBright('database')} system`, "Sagor");
    logger(`deploying ${chalk.blueBright('login')} system`, "Sagor")
    const botData = {};
    botData.models = models;
    onBot(botData);
  } catch (error) { logger(`can't deploy ${chalk.blueBright('database')} system`, "Sagor") }
})();
